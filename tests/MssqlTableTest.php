...
class MssqlTableTest extends \PHPUnit\Framework\TestCase
{
    /**
     * @var PicoDb\Database
     */
    ...
    {
        $this->db = new Database(array('driver' => 'mssql', 'hostname' => getenv('MSSQL_HOST'), 'username' => 'sa', 'password' => 'r00t_Password', 'database' => 'master', 'trust_server_cert' => true));
    ...
    {
        $this->assertEquals('SELECT TOP 10 * FROM [test]', $this->db->table('test')->limit(10)->buildSelectQuery());
        $this->assertEquals('SELECT * FROM [test]', $this->db->table('test')->limit(null)->buildSelectQuery());
    }

    public function testOffset()
    {
        $this->assertEquals('SELECT * FROM [test]       ORDER BY (SELECT NULL) OFFSET 0 ROWS', $this->db->table('test')->offset(0)->buildSelectQuery());
        $this->assertEquals('SELECT * FROM [test]       ORDER BY (SELECT NULL) OFFSET 10 ROWS', $this->db->table('test')->offset(10)->buildSelectQuery());
        $this->assertEquals('SELECT * FROM [test]      ORDER BY [foo] ASC  OFFSET 2 ROWS', $this->db->table('test')->offset(2)->orderBy('foo')->buildSelectQuery());
        $this->assertEquals('SELECT * FROM [test]', $this->db->table('test')->limit(null)->buildSelectQuery());
    }
    ...
    {
        $this->assertNotFalse($this->db->execute('CREATE TABLE foobar_persist (id INT NOT NULL IDENTITY(1,1), a VARCHAR(10), PRIMARY KEY(id))'));
        $this->assertSame(1, $this->db->table('foobar_persist')->persist(array('a' => 'b')));
    }

    public function testInsertUpdate()
    {
        $this->assertNotFalse($this->db->execute('CREATE TABLE foobar (a VARCHAR(10))'));
